...
/**
 *
 * OKL4 Microvisor system call: SCHEDULER_CPU_ADD
 *
 * @param scheduler_id
...
#else
OKL4_FORCE_INLINE okl4_error_t
_okl4_sys_scheduler_cpu_add(okl4_kcap_t scheduler_id,
        okl4_kcap_t physical_cpu_id)
{
    register uint32_t r0 asm("r0") = (uint32_t)scheduler_id;
    register uint32_t r1 asm("r1") = (uint32_t)physical_cpu_id;
    __asm__ __volatile__(
            ""hvc(5208)"\n\t"
            : "+r"(r0), "+r"(r1)
            :
...
{
    register okl4_register_t x0 asm("x0") = (okl4_register_t)scheduler_id;
    register okl4_register_t x1 asm("x1") = (okl4_register_t)physical_cpu_id;
    __asm__ __volatile__(
            "" hvc(5208) "\n\t"
            : "+r"(x0), "+r"(x1)
            :
...
/**
 *
 * OKL4 Microvisor system call: SCHEDULER_CPU_REMOVE
 *
 * @param scheduler_id
 * @param physical_cpu_id
 * @param auto_resume
...
#else
OKL4_FORCE_INLINE okl4_error_t
_okl4_sys_scheduler_cpu_remove(okl4_kcap_t scheduler_id,
        okl4_kcap_t physical_cpu_id, okl4_bool_t auto_resume)
{
    register uint32_t r0 asm("r0") = (uint32_t)scheduler_id;
    register uint32_t r1 asm("r1") = (uint32_t)physical_cpu_id;
    register uint32_t r2 asm("r2") = (uint32_t)auto_resume;
    __asm__ __volatile__(
            ""hvc(5209)"\n\t"
            : "+r"(r0), "+r"(r1), "+r"(r2)
            :
        ...
        okl4_kcap_t physical_cpu_id, okl4_bool_t auto_resume)
{
    register okl4_register_t x0 asm("x0") = (okl4_register_t)scheduler_id;
    register okl4_register_t x1 asm("x1") = (okl4_register_t)physical_cpu_id;
    register okl4_register_t x2 asm("x2") = (okl4_register_t)auto_resume;
    __asm__ __volatile__(
            "" hvc(5209) "\n\t"
            : "+r"(x0), "+r"(x1), "+r"(x2)
            :
...
/**
 *
 * OKL4 Microvisor system call: SCHEDULER_GET_ONLINE_CPUS
 *
 * @param scheduler_id
 *
 * @retval error
...
#elif defined(__ADS__)
#else
OKL4_FORCE_INLINE struct _okl4_sys_scheduler_get_online_cpus_return
_okl4_sys_scheduler_get_online_cpus(okl4_kcap_t scheduler_id)
{
    typedef union {
        struct uint64 {
            uint32_t lo;
    ...
    okl4_uint64_tmp cpu_bitmap_tmp;
    struct _okl4_sys_scheduler_get_online_cpus_return result;
    ...
    result.error = (okl4_error_t)(r0);
    cpu_bitmap_tmp.words.lo = r1;
    cpu_bitmap_tmp.words.hi = r2;
    result.cpu_bitmap = (okl4_register_t)(cpu_bitmap_tmp.val);
    return result;
}
#endif
...
_okl4_sys_scheduler_get_online_cpus(okl4_kcap_t scheduler_id)
{
    ...
    result.error = (okl4_error_t)(x0);
    result.cpu_bitmap = (okl4_register_t)(x1);
    return result;
}
...
#define OKL4_SYSCALL_SCHEDULER_CPU_ADD 5208

#define OKL4_SYSCALL_SCHEDULER_CPU_REMOVE 5209

#define OKL4_SYSCALL_SCHEDULER_GET_ONLINE_CPUS 5210
...
/*lint -esym(621, _okl4_sys_scheduler_cpu_add) */
/*lint -esym(621, _okl4_sys_scheduler_cpu_remove) */
/*lint -esym(621, _okl4_sys_scheduler_get_online_cpus) */
